...
DBNAME="aroundthetable-dev"
MONGO_ROOT="$HOME/mongo32"
...
DB_PORT="27018"
DB_AFFINITY=""
DB_ADAPTERS="127.0.0.1,192.168.0.67"
...
function start {
  ...
  # Create database
  mkdir ${DB_DIR}
  echo ${DB_AFFINITY} ${MONGO_ROOT}/bin/mongod --dbpath ${DB_DIR} --logpath ${LOGFILE} --pidfilepath ${PIDFILE} --port ${DB_PORT} --bind_ip ${DB_ADAPTERS} --nojournal --fork
  ...
  # Populate database
#  ${MONGO_ROOT}/bin/mongo --port ${DB_PORT} < $SCRIPT_DIR/create.js
  unzip dump.zip -d ${MOUNTPOINT}
  ${MONGO_ROOT}/bin/mongorestore --port ${DB_PORT} --db=${DBNAME} ${MOUNTPOINT}/dump/${DBNAME}
}
